                            ...
                            <v-flex md4>
                                <v-layout column class="stat_layout" fill-height>
                                    ...
                                    <v-layout>
                                        <v-flex>
                                            <v-card-text
                                                    v-if="parsers.data.parimatch && parsers.data.parimatch.odds && parsers.data.parimatch.odds.left"
                                                    class="main_stat_item">{{parsers.data.parimatch.odds.left}}
                                            </v-card-text>
                        ...
                        <v-layout column fill-height class="stat_layout" v-if="$store.state.playersBets[dataId]">
                            ...
                            <v-flex pa-2>
                                <v-layout  class="csgo_scoreboard">
                                    <v-flex >PM: <span v-if="parsers.data.parimatch" style="font-size: 20px; color: yellow; margin-left: 10px">{{parsers.data.parimatch.teams.left.score}} : {{ parsers.data.parimatch.teams.right.score }}</span>
                                    </v-flex>
                                    <v-flex>B3: <span v-if="parsers.data.bet365" style="font-size: 20px; font-weight: bold; color: #c7a585; margin-left: 10px">{{parsers.data.bet365.teams.left.score}} : {{ parsers.data.bet365.teams.right.score }}</span>
                                    </v-flex>
                                </v-layout>
                            </v-flex>
                        </v-layout>
                            ...
                            <v-flex md4>
                                <v-layout column class="stat_layout" fill-height>
                                    ...
                                    <v-layout>
                                        <v-flex>
                                            <v-card-text
                                                    v-if="parsers.data.parimatch && parsers.data.parimatch.odds && parsers.data.parimatch.odds.right"
                                                    class="main_stat_item">{{parsers.data.parimatch.odds.right}}
                                            </v-card-text>
                                            <v-card-text v-else class="main_stat_item">Block</v-card-text>
                                        </v-flex>
                                    </v-layout>
...
<script>
  ...
  export default {
    ...
    computed: {
      ...
      mapNumber() {
        if (this.statusString.includes('LIVE на карту')) {
          return this.statusString.substr(-1)
        } else if (this.statusString.includes('LIVE на матч')) {
          return 'winner'
        }
